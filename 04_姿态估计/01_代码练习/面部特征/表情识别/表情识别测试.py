...
class ExpressionRecognitionTest:
    ...
    def draw_ui(self, frame, detected, expression=None):
        h, w, _ = frame.shape
        ...
        if detected and expression:
            expression, color = expression
            status_text = f"  Expression: {expression}"
            cv2.putText(frame, status_text, (20, h - 18), cv2.FONT_HERSHEY_DUPLEX, 0.8, color, 2, cv2.LINE_AA)
            cv2.circle(frame, (w - 60, h - 25), 8, color, -1)
        else:
        ...
        try:
            while True:
                ...
                if result.face_landmarks:
                    ...
                    if expression:
                        exp_text, color = expression
                        cv2.putText(frame, exp_text, (50, 150), cv2.FONT_HERSHEY_DUPLEX, 1.5, color, 3, cv2.LINE_AA)

                self.draw_ui(frame, detected, expression)
